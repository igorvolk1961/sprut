...
resourcestring
  ...
  rsCreateCircle='Построение окружностей';
  ...
  rsViewPan='Перетаскивание изображения';
  ...
  rsBuildRelief='Моделирование рельефа местности...';
  rsBuildSectors='Разбивка зоны на секторы';
...
var
  ButtonArray:array[0..ButtonArrayCount-1, 0..5] of integer=
  ((tbSelect,     btnSelectAll,  smoSelectAll,tbsCheck, 1, 1),
                      (-1, btnSelectLabel,  smoSelectLabel, tbsCheck, 0, 1),
                      (-1, btnSelectClosedPolyLine,  smoSelectVolume, tbsCheck, 0, 1),
                      (-1, btnSelectLine,  smoSelectVerticalArea, tbsCheck, 0, 1),
                      (-1, btnSelectCoordNode, smoSelectVerticalLine, tbsCheck, 0, 1),
                      (-1, btnSelectAll,  smoSelectAll, tbsCheck, 0, 1),
  (-1, btnDropDown,  -1, tbsDropDown2, 2, -1),
  (tbCreateLine,           btnCreateLine,  smoCreateLine, tbsCheck, 1, 1),
                      (-1, btnBreakLine,  smoBreakLine, tbsCheck, 0, 1),
                      (-1, btnCreateEllipse,  smoCreateEllipse, tbsCheck, 0, 1),
                      (-1, btnCreateCircle,  smoCreateCircle, tbsCheck, 0, 1),
                      (-1, btnCreateCurvedLine,  smoCreateCurvedLine, tbsCheck, 0, 1),
                      (-1, btnCreateInclinedRectangle,  smoCreateInclinedRectangle, tbsCheck, 0, 1),
                      (-1, btnCreateRectangle,  smoCreateRectangle, tbsCheck, 0, 1),
                      (-1, btnCreateClosedPolyLine,  smoCreateClosedPolyLine, tbsCheck, 0, 1),
                      (-1, btnCreatePolyLine,  smoCreatePolyLine, tbsCheck, 0, 1),
                      (-1, btnCreateLine,  smoCreateLine, tbsCheck, 0, 1),
  (-1, btnDropDown,  -1, tbsDropDown2, 2, -1),
  (tbTransform,            btnMoveSelected,  smoMoveSelected, tbsCheck, 1, 1),
                      (-1, btnBuildSectors,  smoBuildSectors, tbsCheck, 0, 1),
                      (-1, btnBuildRelief,  smoBuildRelief, tbsCheck, 0, 1),
                      (-1, btnOutlineSelected,  smoOutlineSelected, tbsCheck, 0, 1),
                      (-1, btnIntersectSelected,  smoIntersectSelected, tbsCheck, 0, 1),
                      (-1, btnTrimExtendToSelected,  smoTrimExtendToSelected, tbsCheck, 0, 1),
                      (-1, btnScaleSelected,  smoScaleSelected, tbsCheck, 0, 1),
                      (-1, btnRotateSelected,  smoRotateSelected, tbsCheck, 0, 1),
                      (-1, btnMoveSelected,  smoMoveSelected, tbsCheck, 0, 1),
  (-1, btnDropDown,  -1, tbsDropDown2, 2, -1),
  (tbBuild      ,          btnBuildVolume,  smoBuildVolume, tbsCheck, 1, 1),
                      (-1, btnBuildPolylineObject,  smoBuildPolylineObject, tbsCheck, 0, 1),
                      (-1, btnCreateCoordNode,  smoCreateCoordNode, tbsCheck, 0, 1),
                      (-1, btnBuildArrayObject,  smoBuildArrayObject, tbsCheck, 0, 1),
                      (-1, btnBuildLineObject,  smoBuildLineObject, tbsCheck, 0, 1),
                      (-1, btnDivideVolume,  smoDivideVolume, tbsCheck, 0, 1),
                      (-1, btnBuildVerticalArea,  smoBuildVerticalArea, tbsCheck, 0, 1),
                      (-1, btnCreateDoor,  smoDoubleBreakLine, tbsCheck, 0, 1),
                      (-1, btnBuildVolume,  smoBuildVolume, tbsCheck, 0, 1),
  (-1, btnDropDown,  -1, tbsDropDown2, 2, -1),
  (tbZoomIn,               btnZoomIn,    smoZoomIn,   tbsCheck,  2, 1),
  (tbZoomOut,              btnZoomOut,   smoZoomOut,  tbsCheck,  2, 1),
  (tbViewPan,              btnViewPan,   smoViewPan,  tbsCheck,  2, 1),
  (-1, -1,  -1, tbsSeparator, 2, -1),
                      (-1, btnLastView,  smoLastView, tbsButton, 2, 1),
                      (-1, btnZoomSelection,  smoZoomSelection, tbsButton, 2, 1),
  (-1, -1,  -1, tbsSeparator, 2, -1),
                      (-1, btnSnapNone,  smoSnapNone, tbsCheck, 2, 2),
  (tbSnapToLine,           btnSnapOrtToLine,  smoSnapOrtToLine, tbsCheck, 1, 2),
                      (-1, btnSnapToNode,  smoSnapToNode, tbsCheck, 0, 2),
                      (-1, btnSnapToLocalGrid,  smoSnapToLocalGrid, tbsCheck, 0, 2),
                      (-1, btnSnapToMiddleOfLine,  smoSnapToMiddleOfLine, tbsCheck, 0, 2),
                      (-1, btnSnapToNearestOnLine,  smoSnapToNearestOnLine, tbsCheck, 0, 2),
                      (-1, btnSnapOrtToLine,  smoSnapOrtToLine, tbsCheck, 0, 2),
  (-1, btnDropDown,  -1, tbsDropDown2, 2, -1),
  (-1, -1,  -1, tbsSeparator, 2, -1),
                      (-1, btnRedraw,    smoRedraw,   tbsButton, 2, 3),
  (-1, -1,  -1, tbsSeparator, 2, -1),
                      (-1, btnSideView,    smoSideView,   tbsCheck2, 2, 4),
  (-1, -1,  -1, tbsSeparator, 2, -1),
                      (-1, btnPalette,    smoPalette,   tbsCheck2, 2, 5));
