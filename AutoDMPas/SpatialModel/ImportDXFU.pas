...
uses
  SysUtils, Graphics,
  DataModel_TLB, DMServer_TLB,
  SpatialModelLib_TLB;

procedure ImportDXF(FileName:string; const SpatialModel:ISpatialModel);
...
function FindLayerByName(S:string; const SpatialModel:ISpatialModel):IDMElement;
var j:integer;
begin
  j:=0;
  while (j<SpatialModel.Layers.Count) and
        (S<>SpatialModel.Layers.Item[j].Name) do
    inc(j);
  if j<SpatialModel.Layers.Count then
    Result:=SpatialModel.Layers.Item[j]
  else
    Result:=SpatialModel.CurrentLayer as IDMElement;
end;
...
var
  ...
  aLayer:ILayer;
  aLine:ILine;
  FirstCoord, PrevCoord:ICoordNode;
  aLayerE, aLineE, FirstCoordE, PrevCoordE:IDMElement;
...
//  VerticesCount:integer;
  aColor:TColor;
  Lines2, Nodes2, Layers2:IDMCollection2;
  Nodes, Layers:IDMCollection;
  Server:IDataModelServer;
  NodeCount, LayerCount, j, OldState, OldSelectState:integer;
  Layer:ILayer;
  aDataModel:IDataModel;
  DMDocument:IDMDocument;
  DMOperationManager:IDMOperationManager;
  NodeU, LineU, LayerU:IUnknown;
begin
  aDataModel:=SpatialModel as IDataModel;
  Layers2:=SpatialModel.Layers as IDMCollection2;
  Lines2:=SpatialModel.Lines as IDMCollection2;
  Nodes2:=SpatialModel.CoordNodes as IDMCollection2;
  Nodes:=SpatialModel.CoordNodes as IDMCollection;
  Layers:=SpatialModel.Layers as IDMCollection;
  NodeCount:=(Nodes as IDMCollection).Count;
  LayerCount:=(Layers as IDMCollection).Count;

  DMDocument:=(SpatialModel as IDataModel).Document as IDMDocument;
  DMOperationManager:=DMDocument as IDMOperationManager;
  Server:=DMDocument.Server as  IDataModelServer;
  DMOperationManager.StartTransaction(nil, leoAdd, rsImportDXF);
  ...
  while not EOF(F) do begin
    ...
    wfString:
      begin
        ...
        S:=trim(SV);
        if (S<>'ENDSEC') and
          (DXFSection=secBLOCKS) then Continue;
        case Code of
        0:begin
...
//              Lines2.Add(aLineE);
              ...
              aLine.C0:=NodeU as ICoordNode;
//              aLine.C0:=Nodes2.CreateElement(False) as ICoordNode;
//              Nodes2.Add(aLine.C0 as IDMElement);

              DMOperationManager.AddElement( SpatialModel.CurrentLayer as IDMElement,
                         SpatialModel.CoordNodes, '', ltOneToMany, NodeU, True);

              aLine.C1:=NodeU as ICoordNode;
//              aLine.C1:=Nodes2.CreateElement(False) as ICoordNode;
//              Nodes2.Add(aLine.C1 as IDMElement);
              aLine.C0.Z:=0;
              aLine.C1.Z:=0;
            end else
            if S='LWPOLYLINE' then begin
...
//              Elevation:=0;
              ...
              DMOperationManager.AddElement( nil,
                         SpatialModel.Layers, '', ltOneToMany, LayerU, True);

                aLayerE:=LayerU as IDMElement;
//                aLayerE:=Layers2.CreateElement(False);
//                Layers2.Add(aLayerE);
                aLayer:=aLayerE as ILayer;
              end;
          end;
...
//                  Layers2.Add(aLayerE);
        ...
        8:if DXFEntity=entLine then
             aLineE.Parent:=FindLayerByName(S, SpatialModel)
          else if (DXFEntity=entLWPolyLine) or
                  (DXFEntity=entPolyLine) or
                  (DXFEntity=entVertex) then begin
             aLayerE:=FindLayerByName(S, SpatialModel);
             aLayer:=aLayerE as ILayer;
          end;
        end;
      end;
    wfInteger:
      begin
        readln(F, IV);
        WaitingFlag:=wfCode;
        if DXFSection=secBLOCKS then Continue;
        case Code of
        ...
        62:begin
             ...
             if (DXFSection=secTABLES) and
               ...
               (aLine as ISpatialElement).Color:=aColor
           end;
        ...
        75:begin end;      //Curves and smooth surface type (optional; default = 0); integer codes, not bit-coded:
                                ...
                                //8 = Bezier surface
        90:if (DXFEntity=entPolyLine) or
              (DXFEntity=entLWPolyLine) then begin end;
//              VerticesCount:=IV;
        end;
      end;
    wfFloat:
      begin
        ...
        10:if DXFEntity=entLine then
           ...
           if (DXFEntity=entPolyLine) or
              (DXFEntity=entLWPolyLine) then begin
{             inc(VerticesJ);

             if VerticesJ=1 then begin
               PrevCoordE:=Nodes.CreateElement(False);
               PrevCoord:=PrevCoordE as ICoordNode;
               Nodes.Add(PrevCoordE);
               PrevCoord.X:=FV;
               PrevCoord.Z:=Elevation;
               FirstCoordE:=PrevCoordE;
               FirstCoord:=PrevCoord;
             end else begin
               aLineE:=Lines.CreateElement(False);
               aLine:=aLineE as ILine;
               Lines.Add(aLineE);
               aLine.C0:=PrevCoord;
               aLine.Thickness:=Thickness;
               aLineE.Parent:=aLayerE;
               aLine.C1:=Nodes.CreateElement(False) as ICoordNode;
               Nodes.Add(aLine.C1 as IDMElement);
               aLine.C1.Z:=Elevation;
               aLine.C1.X:=FV;
               PrevCoord:=aLine.C1;
               PrevCoordE:=PrevCoord as IDMElement;
             ...
             PrevCoordE:=NodeU as IDMElement;
//             PrevCoordE:=Nodes2.CreateElement(False);
//             Nodes2.Add(PrevCoordE);
             PrevCoord:=PrevCoordE as ICoordNode;
             PrevCoord.Z:=0;
             PrevCoord.X:=FV;
             if VerticesJ=1 then begin
...
//             Lines2.Add(aLineE);
        ...
        20:if DXFEntity=entLine then
               aLine.C0.Y:=FV
           else if {(DXFEntity=entPolyLine) or
                   (DXFEntity=entLWPolyLine) or}
...
//               Elevation:=FV;
  ...
  try
    for j:=NodeCount to Nodes.Count-1 do
      Nodes.Item[j].Selected:=True;
  finally
  ...
  j:=LayerCount;
  aLayerE:=nil;
  while j<Layers.Count do begin
    aLayerE:=Layers.Item[j];
    Layer:=aLayerE as ILayer;
    if Layer.SpatialElements.Count=0 then
      DMOperationManager.DeleteElement(nil, nil, ltOneToMany, aLayerE)
    else begin
      aLayerE:=Layer as IDMElement;
      inc(j)
    end;
  end;

  if aLayerE<>nil then begin
    Server.DocumentOperation(aLayerE as IUnknown, nil, 0, -1);
  end;
end;
