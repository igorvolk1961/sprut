...
type
  ...
  private
    ...
    procedure IntersectHLine(const HLinesCol, InnerLines:IDMCollection);
    ...
    function Build_NewVolume(const AreaOld:IArea;
    ...
    procedure BuildConnectingLines(const C0, C1:ICoordNode;
                                      const Parent:IDMElement;
                                      const ConnectingLineList:TList;
                                      UpdateFlag:boolean;
                                      const InnerLines:IDMCollection);
  protected
...
{ISMOperationManager}
    ...
    procedure MoveAreas(const DestVolume, SourceVolume:IVolume;
                    const aArea:IArea; const CuttingLines:IDMCollection); safecall;
...
{IVolumeBuilder}
  ...
  public
    ...
    property PainterU:IUnknown read Get_PainterU;
    ...
    function BuildVerticalArea(const BLine:ILine;
                        ...
                        UpdateList:TList;
                        const InnerLines:IdmCollection):IDMElement; safecall;
    ...
    procedure DivideVolume(const theVolume: IVolume; Height: Double; ShiftState: Integer;
...
procedure MakeInnerLines(const ParentVolume:IVolume;
                         const InnerLines:IDMCollection;
                         Direction:integer);
...
begin
  ...
  if FLastViewMode=1 then begin
    if (FPainter<>nil) and
       (FPainter.ViewU<>nil) then begin
      OldState:=Get_State;
      Set_State(OldState or dmfCommiting);
      try
        FLastView.Duplicate(FPainter.ViewU as IView);
      finally
        Set_State(OldState);
      end;
    end;
...
begin
  ...
  if (CurrentOperation.GetModelCheckFlag) and
     (DataModel.Errors<>nil) then begin
  ...
  if CurrentOperation.CurrentStep=-1 then begin
    ...
    if N<0 then Exit;
    aDataModel.State:=aDataModel.State or dmfRollBacking;
    TDMTransaction(FTransactions[N]).Clear;
    TDMTransaction(FTransactions[N]).Free;
    aDataModel.State:=aDataModel.State and not dmfRollBacking;
  ...
  if (sProgram and ShiftState)<>0 then begin
    Server.SetControlState(FDrawToolBarIndex, -CurrentOperation.OperationCode,
                           csChecked, 0);
    PrevOperation:=FCurrentOperation;
  ...
  if CurrentOperation is TSMViewOperation then  begin
    ...
    if LastOperation<>nil then
      Server.SetControlState(FDrawToolBarIndex, -LastOperation.OperationCode,
                           csChecked, 1);
  end else begin
  ...
  if (aSnapMode = smoSnapOrtToLine) or
     ...
     if (NLine=nil) or
       ...
       if FAutoSnapNode then begin
         MinDistance:=FSnapDistance*View.RevScaleX;
...
function TCustomSMDocument.Get_PainterU: IUnknown;
begin
  Result:=FPainter
end;

procedure TCustomSMDocument.Set_PainterU(const Value: IUnknown);
begin
  FPainter:=Value as IPainter
end;
...
begin
  ...
  aDataModel.State:=aDataModel.State or dmfLongOperation;
  FOperationStepExecutedFlag:=False;
  FDocumentOperationFlag:=False;
  FDocumentOperationElement:=nil;
  try
  ...
  if (ShiftState and sProgram)<>0 then
    ...
    aDataModel.State:=aDataModel.State and not dmfLongOperation;
...
begin
  ...
  if aMinDistance>0 then
   ...
   MinD:=FSnapDistance*View.RevScaleX;
...
begin
  ...
  aDataModel:=Get_DataModel as IDataModel;
  if (aDataModel<>nil) and ((aDataModel.State and dmfDestroying)=0) then
    for j:=0 to Get_SelectionCount-1 do begin
     ...
     if (aElement.Ref<>nil) and
      ...
      if aElement<>ExceptedElement then begin
        ...
        if (aElement.QueryInterface(ICoordNode, Unk)=0) or
           (aElement.QueryInterface(IImageRect, Unk)=0) then begin
...
begin
  ...
  try
    ...
    View:=FPainter.ViewU as IView;
    TMPView.Duplicate(View);
  finally
    Set_State(OldState);
  end;

  View.Duplicate(FLastView);
...
begin
  ...
  if aDataModel<>nil then
    aDataModel.State:=aDataModel.State or dmfDestroying;
  inherited;
  FLastView:=nil;
  ClearSelectedAreas;
...
procedure TCustomSMDocument.IntersectHLine(const HLinesCol, InnerLines:IDMCollection);
var
  theLines:IDMCollection;
  theLines2:IDMCollection2;

  function FindVolume:IVolume;
  var
    aLineE, Line0E, Line1E, Area0E, Area1E:IDMElement;
    aLine:ILine;
    C0, C1:ICoordNode;
    X0, Y0, Z0, X1, Y1, Z1:double;
    Area0, Area1:IArea;
...
var
  ...
  HLinesCol2, aCurrentLines2:IDMCollection2;
  aSpatialModel:ISpatialModel;
  aLine, aLineNew, theLineNew, theLine:ILine;
  PX,PY,PZ:double;
  NodeNewU:IUnknown;
  aLineNewU, theLineNewU:IUnknown;
  CNewE:IDMElement;
  CNew:ICoordNode;
  aLineNewE, theLineNewE, Line0E, Line1E, Area0E, Area1E, theLineE, aLineE:IDMElement;
...
begin
  if HLinesCol.Count=0 then Exit;
...
*)  
  ...
  HLinesCol2:=HLinesCol as IDMCollection2;

  aCurrentLines:=DataModel.CreateCollection(-1, nil);
  aCurrentLines2:=aCurrentLines as IDMCollection2;

  if (theLines=nil) or
     (theLines.Count=0) then begin
    aLineE:=HLinesCol.Item[k];
    aLine:=aLineE as ILine;
    if  aLineE.Parents.Count=0 then
      LineUpdateAreas(aLine);
    if  aLineE.Parents.Count=0 then begin
      C0:=aLine.C0;
      C1:=aLine.C1;
      aCurrentLines2.Clear;
      aCurrentLines2.Add(aLineE);
      LineDivideArea1(C0, C1, aCurrentLines, aArea, aNewArea, False)
    end;
    Exit;
  end;

  aLCount:=theLines.Count;  // отрезанные куски по второму разу не могут быть пересечены
  k:=0;
  while k<HLinesCol.Count do begin // куски секущей могут пересекать и другие линии
    aLineE:=HLinesCol.Item[k];
    aLine:=aLineE as ILine;
    C0:=aLine.C0;
    C1:=aLine.C1;
    for l:=0 to aLCount-1 do begin
      ...
      if (theC0<>C0)and(theC1<>C0)and(theC0<>C1)and(theC1<>C1) then begin
        X0:=C0.X;
        Y0:=C0.Y;
        Z0:=C0.Z;
        X1:=C1.X;
        ...
        if LineIntersectLine(
                  ...
                  PX,PY,PZ) and
            not(((X0=PX)and(Y0=PY)and(Z0=PZ))or
             ((X1=PX)and(Y1=PY)and(Z1=PZ)))then begin
             //Создание узла
          AddElement( aLineE.Parent,
          ...
          ChangeFieldValue( aLineE, ord(linC1), True, CNewE as IUnknown);
          ChangeFieldValue( theLineE, ord(linC1), True, CNew as IUnknown);

             //Создание линии
          AddElement( aLineE.Parent,
                            aSpatialModel.Lines, '', ltOneToMany, aLineNewU, True);
          aLineNewE:=aLineNewU as IDMElement;
          ChangeFieldValue( aLineNewE, ord(linThickness), True, aLine.Thickness);
          ChangeFieldValue( aLineNewE, ord(linStyle), True, aLine.Style);
          ChangeFieldValue( aLineNewE, ord(linColor), True, (aLineE as ISpatialElement).Color);
          ChangeFieldValue( aLineNewE, ord(linC0), True, CNew as IUnknown);
          ChangeFieldValue( aLineNewE, ord(linC1), True, C1);
          aLineNew:=aLineNewE as ILine;

             //Создание линии
          AddElement(theLineE.Parent,
                           aSpatialModel.Lines, '', ltOneToMany, theLineNewU, True);
          theLineNewE:=theLineNewU as IDMElement;
          ChangeFieldValue( theLineNewE, ord(linThickness), True, theLine.Thickness);
          ChangeFieldValue( theLineNewE, ord(linStyle), True, theLine.Style);
          ChangeFieldValue( theLineNewE, ord(linColor), True, (theLineE as ISpatialElement).Color);
          ChangeFieldValue( theLineNewE, ord(linC0), True, CNew as IUnknown);
          ChangeFieldValue( theLineNewE, ord(linC1), True, theC1);

          for j:=0 to theLineE.Parents.Count-1 do begin
            aParent:=theLineE.Parents.Item[j];
            theLineNewE.AddParent(aParent);
          end;

          theLines2.Add(theLineNewE);
          HLinesCol2.Add(aLineNewE);

          C1:=CNew;  // важно не забыть, что конец изменился!!!
        end;  // if LineIntersectLine
      end; // if (theC0<>C0)and(theC1<>C0)and(theC0<>C1)and(theC1<>C1)
    end;  // for l:=0 to aLCount-1
    inc(k);
  end;  // while k<HLinesCol.Count

  for l:=0 to theLines.Count-1 do begin
    theLineE:=theLines.Item[l];
    for k:=0 to theLineE.Parents.Count-1 do begin
      aElement:=theLineE.Parents.Item[k];
...
var
  ...
  PlanesAreParallel, ForceNodesDeletion:boolean;
...
begin
  ...
  InLongOperation:=(aDataModel.State and dmfLongOperation)<>0;
...
procedure TCustomSMDocument.MoveAreas(const DestVolume, SourceVolume:IVolume;
                    const aArea:IArea; const CuttingLines:IDMCollection);
var
  DestVolumeE, SourceVolumeE, aAreaE, NextAreaE:IDMElement;
  aAreaP:IPolyline;
  aLineE:IDMElement;
  NextArea:IArea;
  j, m:integer;
begin
  ...
  for j:=0 to aAreaP.Lines.Count-1 do begin
    ...
    if CuttingLines.IndexOf(aLineE)=-1 then begin
      m:=0;
      while m<aLineE.Parents.Count do begin
        ...
        if (NextAreaE.QueryInterface(IArea, NextArea)=0) and
          ...
          MoveAreas(DestVolume, SourceVolume, NextArea, CuttingLines)
        else
          inc(m)
      end;
    end;
...
var
  ...
  begin
    ...
    theAreaLines:=(theArea as IPolyline).Lines;
    MoveAreas(theVolume, aVolume,
              BottomArea, theAreaLines);
...
function TCustomSMDocument.BuildVerticalArea(const BLine: ILine;
   Direction: integer; BuildVolumeFlag: WordBool;
   UpdateList:TList;
   const InnerLines:IdmCollection): IDMElement;
...
var
  ...
  begin
    ...
    k:=0;
    while k<InnerLines.Count do begin
      aLineE:=InnerLines.Item[k];
      aLine:=aLineE as ILine;
      aC:=aLine.C0;
      if (abs(aC.X-X)<1) and
         (abs(aC.Y-Y)<1) then
    ...
    if k<InnerLines.Count then begin
      Result:=aC;
      Exit;
    end;
    ...
    if k<InnerLines.Count then begin
      ...
      (InnerLines as IDMCollection2).Add(LineE);
    end;
  end; // FindNode
...
var
  ...
  DividingAreaLines:IDMCollection;
begin
  ...
  HV:=0;
  if aArea=nil then begin
    IntersectHLine(BLinesCol, InnerLines);
  ...
  BuildConnectingLines(C0, C1, BLineE.Parent, ConnectingLineList, True, InnerLines);
  ...
  if (Area0<>nil)
    ...
    DividingAreaLines:=(DividingArea as IPolyline).Lines;
    MoveAreas(NewVolume, Volume, Area1, DividingAreaLines);
...
begin
  ...
  aCurrentLines:=DataModel.CreateCollection(-1, nil);
  aCurrentLines2:=aCurrentLines as IDMCollection2;
  InnerLines:=DataModel.CreateCollection(-1, nil);
  if ParentVolume<>nil then
    MakeInnerLines(ParentVolume, InnerLines, Direction);
...
//!!!!
    ...
    if SideArea=nil then begin
      ...
      BuildConnectingLines(CC0, CC1, BLineE.Parent, ConnectingLineList, False,
                              InnerLines);
...
//................
  ...
  if aArea<>nil then begin
    ...
    aAreaLines:=(aArea as IPolyline).Lines;
    MoveAreas(NewVolume, Volume, BaseArea, aAreaLines);
    UpdateElement( VolumeE);
    UpdateElement( NewVolumeE);
            ...
            for i1:=0 to Line3E.Parents.Count-1 do begin
              if aAreaE=Line3E.Parents.Item[i1] then begin
                if CommonLineFlag then begin
                  if NodeIsInternalCheck(aArea, X1, Y1, Z1, aStat) then begin
                    result:=aArea;
                    Exit;
                  end;
                end else
...
function TCustomSMDocument.Build_NewVolume(const AreaOld:IArea;
                     out Volume:IVolume;out NewVolume:IVolume):WordBool;
      {создание объема}
var
  SpatialModel: ISpatialModel;
...
var
  ...
  j,k,i,m, Index1, Index2, n, aLinesCount, aCount:integer;
  aCurrent:integer;
  SpatialModel:ISpatialModel;
...
var
  ...
  aOppositeLines2:IDMCollection2;
  ...
  GlobalData:IGlobalData;
  UpdateList:TList;
  OldAreas, NewAreas, InnerLines:IDMCollection;
  ErrorCount:integer;
  CheckErrorsFlag:boolean;
  CuttingLines:IDMCollection;
  CuttingLines2:IDMCollection2;
  VAreaP:IPolyline;
begin
  ...
  FindEndedNodes(PolyLineCollection,C2,C3,C0); {определить конечн.точки полилин.}
...
//--------------------------------------------------------------------------
  ...
  if aLineE.Parents.Count>0 then begin
    ...
    while j1<aLineE.Parents.Count do begin
      aAreaE:=aLineE.Parents.Item[j1];
      if (aAreaE.QueryInterface(IArea, aArea)=0) and
         (not aArea.IsVertical) then begin
        if aBaseOldAreaE=nil then begin
          aBaseOldAreaE:=aAreaE;
          inc(j1);
        end else begin
          aBaseNewAreaE:=aAreaE;
          Break
        end;
      end else
        inc(j1);
    ...
    if aBaseOldAreaE=nil then Exit;
    if aBaseNewAreaE=nil then Exit;

    aBaseOldArea:=aBaseOldAreaE as IArea;
    aBaseNewArea:=aBaseNewAreaE as IArea;
    ...
    if aNewVolume=nil then
      Build_NewVolume(aBaseOldArea,
                    aOldVolume,aNewVolume);      //NewVolume
  ...
  aOppositeLines2:=aOppositeLines as IDMCollection2;
  ...
  InnerLines:=DataModel.CreateCollection(-1, nil);
  MakeInnerLines(aOldVolume, InnerLines, BuildDirection);

  UpdateList:=TList.Create;

  CuttingLines:=DataModel.CreateCollection(-1, nil);
  CuttingLines2:=CuttingLines as IDMCollection2;
  Index1:=0;
  while  Index1 < aLinesCount do begin
       aCurrentLineE:=IDMElement(PolyLineCollection.Item[Index1]);
       ...
       if aCurrentLine.GetVerticalArea(BuildDirection)=nil then
         VAreaE:=BuildVerticalArea(aCurrentLine,BuildDirection,
                                   BuildVolumeFlag, UpdateList, InnerLines);
       if (VAreaE<>nil) then  begin
        ...
        //----------
         ...
         VAreaP:=VAreaE as IPolyline;
         for k:=0 to VAreaP.Lines.Count-1 do begin
           aLineE:=VAreaP.Lines.Item[k];
           i:=CuttingLines.IndexOf(aLineE);
           if i=-1 then
             CuttingLines2.Add(aLineE)
           else
             CuttingLines2.Delete(i)
         end;

         for k:=0 to VArea.TopLines.Count-1 do begin
           aLineE:=VArea.TopLines.Item[k];
           aOppositeLines2.Add(aLineE);
         end;
       end;  //if (VArea<>nil)
    inc(Index1);
  end;   //while
  ...
  MoveAreas(aNewVolume, aOldVolume,  aBaseNewArea, CuttingLines);
...
procedure TCustomSMDocument.BuildConnectingLines(const C0, C1:ICoordNode;
                                                    const Parent:IDMElement;
                                                    const ConnectingLineList:TList;
                                                    UpdateFlag:boolean;
                                                    const InnerLines:IDMCollection);
var
  ...
  DataModel:IDataModel;
  SpatialModel:ISpatialModel;
  HLinesCol, aCurrentLines:IDMCollection;
  aArea, aNewArea:IArea;
  aCurrentLines2:IDMCollection2;
begin
  ...
  while True do begin
    ...
    while k<aC1.Lines.Count do begin
      ...
      if aC0=aLine.NextNodeTo(aC1) then
        Break    // Нашли общую линию - она и замыкает цепочку
      else
        inc(k);
    end;
    if k=aC1.Lines.Count then begin  // Общую линию не нашли, ищем сонаправленные
      aLineE:=nil;
      X:=0;
      Y:=0;
      k:=0;
      ...
      if k<aC1.Lines.Count then begin
        ...
        aC1:=aC;        // нашли в C1 линию, направленную к C0
        X1:=X;
        Y1:=Y;
      end else
        Break      // нет сонаправленных линий
    end else begin // if k=aC1.Lines.Count
      ConnectingLineList.Add(pointer(aLineE)); // нашли замыкающую линию
      for k:=0 to ConnectingLineList.Count-1 do begin
        ...
        if  aLineE.Parents.Count=0 then begin
          ...
          aCurrentLines2.Clear;
          aCurrentLines2.Add(aLineE);
          LineDivideArea1(aC0, aC1, aCurrentLines, aArea, aNewArea, False)
        end
      end;
      Exit;
    end;
  ...
  while True do begin  // повторим поиск из узла C0  по направлению к С1
    aLineE:=nil;
    X:=0;
    Y:=0;
    k:=0;
    ...
    if k<aC0.Lines.Count then begin
      ConnectingLineList.Add(pointer(aLineE));
      aC0:=aC;          // нашли в C0 линию, направленную к C1
      X0:=X;
      Y0:=Y;
    end else
      Break      // нет сонаправленных линий
  end; // while True
                // Соединыем разомкнутую цепочку
  ...
  IntersectHLine(HLinesCol, InnerLines); // внутри вызывается LineDivideArea1
  for k:=0 to HLinesCol.Count-1 do begin
    aLineE:=HLinesCol.Item[k];
    ConnectingLineList.Add(pointer(aLineE));
  end;
...
procedure TCustomSMDocument.DivideVolume(const theVolume: IVolume; Height: Double; ShiftState:
...
var
  ...
  DataModel:IDataModel;
  NewAreaLines:IDMCollection;
  NextVolume, Volume:IVolume;
begin
  DataModel:=Get_DataModel as IDataModel;
  SpatialModel:=DataModel as ISpatialModel;
  SpatialModel2:=SpatialModel as ISpatialModel2;
  ...
  for L:=0 to N-2 do begin
    ...
    for m:=0 to LineList.Count-1 do begin
      ...
      if VLine<>nil then begin
        if abs(Length-aHeight)>1 then begin
      ...
      if VLine<>nil then begin
        ...
        if LastNode<>nil then begin
          BuildConnectingLines(LastNode, aC1, BLineE.Parent,
                                  ConnectingLineList, True, nil);
          for k:=0 to ConnectingLineList.Count-1 do begin
            aLineE:=IDMELement(ConnectingLineList[k]);
    ...
    if SkipFlag0 and
      ...
      (LastNode<>nil) then begin
       ...
       BuildConnectingLines(LastNode, FirstNode, BLineE.Parent,
                               ConnectingLineList, True, nil);
       for k:=0 to ConnectingLineList.Count-1 do begin
         aLineE:=IDMELement(ConnectingLineList[k]);
         aLineE.AddParent(NewAreaE);
    ...
    if aLineE<>nil then begin
      ...
      VArea:=aLine.GetVerticalArea(BuildDirection);
      if VArea<>nil then begin
        NewAreaLines:=(NewArea as IPolyline).Lines;
        MoveAreas(NewVolume, Volume, VArea, NewAreaLines);
      end;
    end;
...
function FindWallNodes(var C0, C1:ICoordNode;
                         ...
                         const LineList0, LineList1:TList;
                         UntillWall:boolean):boolean;
var
  NodeRecList:TList;
  ...
  var
    ...
    Line, aLine:ILine;
    LineE, aLineE:IDMElement;
    j:integer;

    function HorizontalArea(const aLineE:IDMElement):IDMELement;
    var
      aArea:IArea;
      m:integer;
      aAreaE:IDMElement;
    begin
      ...
      if m<aLineE.Parents.Count then
        Result:=aAreaE
    end;
  begin
    Result:=False;
    ...
    while j<Node.Lines.Count do begin
      ...
      aLine:=aLineE as ILine;
      if UntillWall then begin
        if aLine.GetVerticalArea(BuildDirection)<>nil then
          Break
        else
          inc(j)
      end else begin
        if HorizontalArea(aLineE)<>nil then
          Break
        else
          inc(j)
      end;
...
procedure TCustomSMDocument.LineDivideArea1(var C0, C1:ICoordNode; const aCurrentLines:IDMCollection;
      var aArea, aNewArea: IArea; SearchWall:boolean);
var
  LineList0, LineList1:TList;
  aC2, aC0, aC1, bC0, bC1:ICoordNode;
  Flag:boolean;
  m:integer;
  aLineE:IDMElement;
  DataModel:IDataModel;

  function CutPolyline(const aCurrentLines:IDMCollection; const C:ICoordNode;
                       out aLineE:IDMElement):ICoordNode;
  var
    m:integer;
    aLine:ILine;
  begin
    ...
    while m<C.Lines.Count do begin
      ...
      if aCurrentLines.IndexOf(aLineE)<>-1 then
        Break
      else
        inc(m)
    end;
    if m<C.Lines.Count then begin
      aLine:=aLineE as ILine;
      Result:=aLine.NextNodeTo(C);
    end else
      Result:=nil
  end;
...
begin
  ...
  aC0:=C0;
  aC1:=C1;
  Flag:=FindWallNodes(aC0, aC1, aCurrentLines, LineList0, LineList1, SearchWall);

  if LineList0.Count>0 then begin
    m:=LineList0.Count div 2;
    aLineE:=IDMElement(LineList0[m]);
    aC2:=(aLineE as ILine).C0;
    if (aC2=aC0) or (aC2=aC1) then
      aC2:=(aLineE as ILine).C1;
  end else
  ...
  if Flag then begin
    aArea:=Find_DivideAreaWithNodes(aC0,aC1,aC2, aCurrentLines, aNewArea);
    ...
    if aCurrentLines.Count>1 then begin
      Line:=aCurrentLines.Item[0] as ILine;
      Line1:=aCurrentLines.Item[1] as ILine;
      if (Line.C0=Line1.C0) or (Line.C0=Line1.C1) then
        aC0:=Line.C1
      else
        aC0:=Line.C0;
      Line:=aCurrentLines.Item[aCurrentLines.Count-1] as ILine;
      Line1:=aCurrentLines.Item[aCurrentLines.Count-2] as ILine;
      if (Line.C0=Line1.C0) or (Line.C0=Line1.C1) then
        aC1:=Line.C1
      else
        aC1:=Line.C0;
...
var
  ...
  k, m1, i:integer;
  aLineE:IDMElement;
  InnerLines2:IDMCollection2;
begin
  if Direction=bdUp then
    Areas:=ParentVolume.TopAreas
  else
    Areas:=ParentVolume.BottomAreas;
  OuterLineList:=TList.Create;
  InnerLines2:=InnerLines as IDMCollection2;
  for k:=0 to Areas.Count-1 do begin
    aAreaP:=Areas.Item[k] as IPolyline;
    for  m1:=0 to aAreaP.Lines.Count-1 do begin
      aLineE:=aAreaP.Lines.Item[m1];
      i:=InnerLines.IndexOf(aLineE);
      if i=-1 then
        InnerLines2.Add(aLineE);
  ...
  for m1:=0 to OuterLineList.Count-1 do begin
    ...
    i:=InnerLines.IndexOf(aLineE);
    if i<>-1 then
      InnerLines2.Delete(i);
  end;
  OuterLineList.Free;
end;

function TCustomSMDocument.Get_DrawToolBarIndex:integer;
begin
  Result:=FDrawToolBarIndex
end;

procedure TCustomSMDocument.Set_DrawToolBarIndex(Value:integer);
begin
  FDrawToolBarIndex:=Value
end;
