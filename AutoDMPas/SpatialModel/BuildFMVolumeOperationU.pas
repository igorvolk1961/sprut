...
procedure ExitWithMessage(Mode:integer);
...
procedure  ExitWithMessage(Mode:integer);
begin
 ShowMessage(Format('Задано недопустимое построение (%d)',[Mode]));
 Exit;
...
var
  ...
  var
    ...
    var
      j, i, k:integer;
      Area, aArea:IArea;
      theAreaE, SomeAreaE:IDMElement;
      theAreaP, aAreaP:IPolyline;
      LineE:IDMELement;
      Line:ILine;
      C0, C1:ICoordNode;
    ...
    begin
      ...
      j:=0;
      while j<theAreaP.Lines.Count do begin
        LineE:=theAreaP.Lines.Item[j];
        if LineList.IndexOf(pointer(LineE))=-1 then
          Break
        else
          inc(j)
      end;
      if j<theAreaP.Lines.Count then
        Exit;
      ...
      for j:=0 to theAreaP.Lines.Count-1 do begin
        LineE:=theAreaP.Lines.Item[j];
        if BuisyList.IndexOf(pointer(LineE))=-1 then begin
        ...
        if i=-1 then begin
          ...
          if aArea<>nil then begin
            ...
            if Area=nil then begin
              ...
              k:=0;
              while k<SelectedAreaList.Count do begin
                SomeAreaE:=IDMELement(SelectedAreaList[k]);
                if ((SomeAreaE as IPolyline).Lines as IDMCollection2).IsEqualTo(aAreaP.Lines) then
                  break
                else
                  inc(k)
              end;
              if k=SelectedAreaList.Count then
  ...
  var
    ...
    theArea, Area:IArea;
    C0, C1:ICoord;
    X, Y, X0, Y0, Z0, X1, Y1, Z1, ProjL, cosX, cosY, W0X, W0Y, W1X, W1Y:double;
  ...
  begin
    ...
    for m:=0 to LineList.Count-1 do begin
      LineE:=IDMElement(LineList[m]);
      if UsedList.IndexOf(pointer(LineE))=-1 then begin
        Line:=LineE as ILine;
        C0:=Line.C0;
        C1:=Line.C1;
        X0:=C0.X;
        ...
        if (theArea=nil) or
           (SpatialModel2.GetAreaEqualTo(theArea)<>nil) then begin
          theX:=W1X;
          theY:=W1Y;
          theArea:=SpatialModel2.BuildAreaSurrounding(theX, theY, theZ);
          if (theArea=nil) or
             (SpatialModel2.GetAreaEqualTo(theArea)<>nil) then
             theArea:=nil;
        end;
